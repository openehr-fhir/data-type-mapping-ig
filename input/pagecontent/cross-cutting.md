...
| Compact time, hours and minutes only <br/>*(`time`)* | `T1430` | ✓ | — | SHALL be expanded to `14:30` and then completed to `14:30:00`. Neither the compact form nor a seconds-less time is a FHIR `time`, and the completion adds a precision the source did not state, so it is a **named drop**. |
...
| Compact time with a compact UTC offset <br/>*(`time`)* | `T143000+0100` | ✓ | — | SHALL be expanded to `14:30:00+01:00` before the offset is separated from the time; FHIR accepts neither the compact time nor the compact offset, and then has no home for the offset at all. |
